...
export IMAGE_REPO='ccr.ccs.tencentyun.com/joysafety/joysafety'
...
cd $deploy_dir/../safety/safety-skills/safety-keywords/build \
  && docker run  \
    ...
    $IMAGE_REPO':safety-keywords-0.0.1'
echo '启动keywords容器结束'
...
cd $deploy_dir/../safety/safety-skills/safety-fasttext/build \
  && docker run  \
       ...
       $IMAGE_REPO':safety-fasttext-0.0.1'
echo '启动fasttext容器结束'
...
cd $deploy_dir/../safety/safety-skills/safety-knowledge/build \
  && docker run  \
       ...
       $IMAGE_REPO':safety-knowledge-0.0.1'
